...
date: 25 Maja, 2017 <br> <a href="https://www.meetup.com/pl-PL/Spotkania-Entuzjastow-R-Warsaw-R-Users-Group-Meetup/events/239276697/">SER 26</a>
output:
...
<small>
Autor: [Alboukadel Kassambara](https://github.com/kassambara), Pobrań: 28 tysięcy, 
...
```{r}
ggrisktable(fit, color = "strata",  y.text = FALSE)
```
...
```{r}
surv <- Surv(
   BRCAOV.survInfo$times,               # times
   BRCAOV.survInfo$patient.vital_status # events status
)
ggsurvevents(surv)
```
...
```{r}
ggcumevents(fit, color = "strata", y.text = FALSE)
```
...
```{r}
ggcumcensor(fit, color = "strata", y.text = FALSE)
```
...
<small>
Jeżeli zaciekawił Cię temat, to zapraszam na swoją prezentację <br> [Stochastic Gradient Descent Log-Likelihood Estimation in the Cox Proportional Hazards Model with Applications to The Cancer Genome Atlas Data](https://github.com/MarcinKosinski/useR2017/blob/master/abstract_MarcinKosinski.md) <br> podczas konferencji useR 2017, Bruksela 4-7.07.2017.
</small>
